...
let UTIL = {
    ...
    coordsToQuadrant: (coords) => {
        ...
        let [xMid, yMid] = UTIL.getAvgCenter();
        xMid = 0;
        if(x > xMid){
            if(y > yMid){ 
                quadrant = 1
    ...
    getAvgCenter: () => {
        ...
        return [xTotal/UTIL.centerList.length, yTotal/UTIL.centerList.length]
    },
    ...
    getModelInput: () => {
        ...
        [wholeImage, imageLeft, imageRight, imageFace] = UTIL.getModelPics(features);
        let faceGrid = UTIL.createFaceGridFromBox(wholeImage, features['face'], features['faceGridPoints']);
        // [imageLeft, imageRight, imageFace] = [UTIL.reshapeImageData(imageLeft),
         // UTIL.reshapeImageData(imageRight), UTIL.reshapeImageData(imageFace)]
	...
	setNewSequence: (seqLen) => {
		UTIL.sequence = [];
		UTIL.sequence.push(Math.floor(Math.random() * 4) + 1)
        for(var i=1; i < seqLen; i++){
            var choose = (Math.random() < 0.5) ? -1 : 1;
            var newQuadrant = (UTIL.sequence[i-1] + choose) % 4
            if(newQuadrant == 0){
                newQuadrant = 4; 
            }
            UTIL.sequence.push(newQuadrant);
        }
        DISPLAY.showSequenceText(UTIL.sequence);
    ...
    updateScore: (scoreChange) => {
        UTIL.score += scoreChange;
        DISPLAY.displayScore(UTIL.score);
    },

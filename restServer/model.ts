...
export class CaffeModel implements Model {
  ...
  predict(
      input: Array<NDArray>, untilLayer?: string,
    ...
    let currAct: NDArray|NDArray[] = input[0];
    let meanImgKey: string = null;

    dag.iterateDfs<caffe.ILayerParameter>(
        this.nodes, this.edges,
        (layer: caffe.ILayerParameter, parents: caffe.ILayerParameter[],
         i: number, depth: number) => {
            if ( i === 0 || layer.name == 'image_left' || layer.name == 'image_right' || 
            layer.name == 'image_face' || layer.name == 'facegrid') {
            meanImgKey = null;

            switch (layer.name){
                case 'image_left':
                    currAct = input[0];
                    meanImgKey = 'mean_left';
                    break;
                case 'image_right':
                    currAct = input[1];
                    meanImgKey = 'mean_right';
                    break;
                case 'image_face':
                    currAct = input[2];
                    meanImgKey = 'mean_face';
                    break;
                case 'facegrid':
                    currAct = input[3];
                    break;
  ...
  //              currAct = math.reverse3D(currAct as Array3D, -1);
            ...
            if(meanImgKey != null){
              currAct =
                  math.subtract(currAct as Array3D, (this.variables[meanImgKey][0] as Array3D)) as
                  Array3D;
            }else if (layer.name != 'facegrid' && this.preprocessOffset  &&
          ...
          if(layer.name == 'fc1_f' || layer.name == 'fc1'){
              currAct = math.transpose(currAct as Array3D, [2,1,0]);
          }        
          currAct = performMathOp(
              math, currAct, layer, this.variables[`${layer.name}`]);
          if(meanImgKey == null && layer.name != 'fc3'){
            currAct = math.clip(currAct, 0, 10000)
          }
